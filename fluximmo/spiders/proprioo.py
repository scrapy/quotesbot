...
class propriooSpider(CrawlSpiderFluximmo):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.link_extractor_annonces = LinkExtractor(
            allow=r"https:\/\/www\.proprioo\.fr\/nosannonces\/annonce\/([a-zA-Z-0-9._%]*)\/([0-9]*)",
        )
    ...
    def parse_item(self, response):
        ...
        try:
            others += response.xpath(f"{ROOT_XPATH}/*[contains(@aria-label, 'performance énergétique')]/div/*[contains(@role, 'group')][1]/@aria-label").extract_first()
        except:pass
        try:
            others += response.xpath(f"{ROOT_XPATH}/*[contains(@aria-label, 'performance énergétique')]/div/*[contains(@role, 'group')][2]/@aria-label").extract_first()
        except:pass
